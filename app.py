...
def admin_productos():
    ...
    return render_template('admin/productos.html', productos=productos, subcategorias=subcategorias)
...
def agregar_producto():
    if request.method == 'POST':
        try:
            ...
            costo = float(request.form.get('costo', 0)) or None
    ...
    if request.method == 'POST':
        try:
            if not producto.proveedor_id:
                ...
                margen = ((producto.precio - producto.costo) / producto.costo) * 100
                if margen < 50:
